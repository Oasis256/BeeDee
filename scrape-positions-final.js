...
async function scrapePositionsFinal() {
  ...
  try {
    for (const position of positionUrls) {
      console.log(`\n🔍 Scraping positions with content from: ${position.category}`);
      ...
      try {
        ...
        const enhancedData = await page.evaluate(() => {
          ...
          positionHeadings.forEach((heading, index) => {
            ...
            if (allText.length > 0) {
              // Clean up the text - remove social media handles and clean up formatting
              const cleanTexts = allText.map(text => {
                // Remove social media handles like @LUCYMACARONI
    ...
    fs.writeFileSync('all-positions-with-content.json', JSON.stringify(allDetailedPositions, null, 2));
    ...
    fs.copyFileSync('all-positions-with-content.json', 'public/all-positions-with-content.json');
    console.log(`\n📁 Copied to public/all-positions-with-content.json for frontend use`);
